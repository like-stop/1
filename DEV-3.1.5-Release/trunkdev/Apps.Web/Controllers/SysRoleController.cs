...
namespace Apps.Web.Controllers
{
    public class SysRoleController : BaseController
    {
        ...
        public ActionResult GetUserByRole(string roleId)
        {
            ...
            ViewBag.DepId = GetDepId();
            CommonHelper commonHelper = new CommonHelper();
            ViewBag.StructTree = commonHelper.GetStructTree(true);

            var userList = m_BLL.GetUserByRoleId(roleId);
            string fs = "";
        ...
        public JsonResult GetUserListByRole(GridPager pager, string roleId, string depId, string queryStr)
        {
            ...
            //查询出所有已经选中用户ID
            List<SysUserModel> ul = new List<SysUserModel>();
            string fs = "";
            foreach (var r in userList)
            {
                ul.Add(new SysUserModel()
                {
                    Id = r.Id,
                    UserName = r.UserName,
                    TrueName = r.TrueName,
