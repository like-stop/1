...
namespace Apps.Core
{
    public static class LoginUserManage
    {
        ...
        public static void RedirectUrlFor401()
        {
            ...
            HttpContext.Current.Response.Write("401 页面未授权,请联系管理员授权！");
            HttpContext.Current.Response.End();
        ...
        public static bool ValidateTicket(string encryptToken)
        {
            ...
            SysValidTokenRepository m_Rep = new SysValidTokenRepository(new Models.DBContainer());


            //对比
            SysValidToken myToken = m_Rep.GetById(userName);
            if (myToken != null && myToken.OverTime >= ResultHelper.NowTime)
            {
                //更新过期时间
                myToken.OverTime = ResultHelper.NowTime.AddDays(7);
                m_Rep.Edit(myToken);
                return true;
            }
            else
